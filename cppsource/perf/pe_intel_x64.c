#include "pe_assist.h"
#include "pe_intel_x64.h"

static void x64Error(char* fn, char* event)
{
 ...
 *
 */
int x64MeasureInstructionEvents()
{
  ...
  if (peMakeGroup(6, 0, -1, 0) == 0) {
    fprintf(stderr,
	    "Couldn't make event group (x64MeasureInstructionEvents)\n") ;
    exit( EXIT_FAILURE ) ;
  }
 ...
 */
int x64MeasureDataAccessEvents()
{
  // Initialize the event helper module
  peInitialize() ;
  ...
  // Make an event group (two events, monitor this PID, monitor all CPUs)
  // Request 6 events, monitor this PID, monitor all CPUs
  if (peMakeGroup(4, 0, -1, 0) == 0) {
    fprintf(stderr, "Couldn't make group (x64MeasureDataAccessEvents)\n") ;
    exit( EXIT_FAILURE ) ;
  }

  // Add leader event to the group
  // Raspberry Pi 4 (Cortex-A72) event 0x08 (instructions retired)
  if (peAddLeader(PERF_TYPE_RAW, X64_INST_RETIRED) == 0) {
    x64Error("x64MeasureDataAccessEvents", "instructions") ;
    exit( EXIT_FAILURE ) ;
  }

  // Add event to the group
  // Raspberry Pi 4 (Cortex-A72) event 0x11 (cpu cycles)
  if (peAddEvent(PERF_TYPE_RAW, X64_CPU_CYCLES) == 0) {
    x64Error("x64MeasureDataAccessEvents", "CPU cycles") ;
    exit( EXIT_FAILURE ) ;
  }

  // Add event to the group
  // Raspberry Pi 4 (Cortex-A72) event 0x04 (L1 D-cache access)
  if (peAddEvent(PERF_TYPE_RAW, X64_L1D_CACHE_ACCESS) == 0) {
    x64Error("x64MeasureDataAccessEvents", "L1 D-cache access") ;
    exit( EXIT_FAILURE ) ;
  }

  // Add event to the group
  // Raspberry Pi 4 (Cortex-A72) event 0x03 (L1 D-cache refill)
  if (peAddEvent(PERF_TYPE_RAW, X64_L1D_CACHE_REFILL) == 0) {
    x64Error("x64MeasureDataAccessEvents", "L1 D-cache miss") ;
    exit( EXIT_FAILURE ) ;
  }

  // Add event to the group
  // x86_64 event 0x24 (L2 cache access read)
  // See https://stackoverflow.com/a/16078495 and ../code/x64-perf.txt
  // TODO - get the right event (not just a copy of L1)
  // if (peAddEvent(PERF_TYPE_HARDWARE, PERF_COUNT_HW_BUS_CYCLES) == 0) {
...
#ifndef X64_EXCLUDE_PRINTS

int x64PrintDataAccessEvents(FILE* stream)
{
  // Read instruction event counts
...
static const char* x64DataAccessEventNames[] = {
  "instructions",
  "cpu_cycles",
  "d_cache_reads",
  "d_cache_misses",
...
static const char* x64DataAccessEventTitles[] = {
  "Instructions",
  "CPU cycles",
  "L1 data cache reads",
...
static const char* x64DataAccessRatioNames[] = {
  "instructions_per_cycle",
  "l1_dcache_reads_pti",
  "l1_dcache_miss_ratio",
};

static const char* x64DataAccessRatioTitles[] = {
  "Instructions per cycle",
...
extern void x64CollectDataAccessEvents(pe_event_collection* c) {
  ...
  pe_event_collection_populate(c, 
    4, x64DataAccessEventNames, x64DataAccessEventTitles, 
    3, x64DataAccessRatioNames, x64DataAccessRatioTitles
  );
